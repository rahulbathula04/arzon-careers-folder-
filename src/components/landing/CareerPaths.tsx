...
export function CareerPaths() {
  ...
  const trackColor = TRACK_COLORS[active.id] ?? {
    text: "text-[#1B3F8B]",
    bg: "bg-sky-50",
      ...
      <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8 space-y-10">
        ...
        <div className="text-center space-y-4 max-w-3xl mx-auto">
          ...
          <p className="text-sm sm:text-base text-stone-700 leading-relaxed font-sans font-medium">
            Every career path below is built from real Indian job descriptions. Select a role to see
            what skills it requires, what the work actually involves, and what Arzon modules build
            that readiness.
          </p>
        </div>
            ...
            return (
              ...
              >
                ...
                {track.status === "coming_soon" && (
                  <span className="ml-1.5 text-[9px] font-mono font-bold text-stone-400">
                    ·Coming
                  </span>
                )}
              </button>
        ...
        <div className="bg-white tone-light card-light rounded-3xl border border-stone-200 p-6 sm:p-8 shadow-xl grid grid-cols-1 lg:grid-cols-12 gap-8">
          ...
          <div className="lg:col-span-6 space-y-6 border-r-0 lg:border-r border-stone-100 lg:pr-8">
            ...
            <div className="space-y-3 border-b border-stone-100 pb-5">
              <span
                className={`inline-block text-[11px] font-mono font-bold px-3 py-1 rounded-full border ${trackColor.bg} ${trackColor.border} ${trackColor.text}`}
              >
                {active.heroBadge}
              </span>
            ...
            <div className="grid grid-cols-2 gap-3">
              <div className="bg-stone-50 p-3.5 rounded-xl border border-stone-200">
                ...
                <span className={`font-serif font-bold text-base mt-0.5 block ${trackColor.text}`}>
                  {active.salaryRange}
                </span>
              </div>
              <div className="bg-stone-50 p-3.5 rounded-xl border border-stone-200">
                ...
                <span className={`font-mono font-bold text-sm mt-0.5 block ${trackColor.text}`}>
                  {active.entryLevelListings}
                </span>
              </div>
            </div>
            ...
            <div className="space-y-2">
              ...
              <div className="flex flex-wrap gap-2">
                {active.sampleEmployers.map((e) => (
                  <span
                    key={e}
                    className="text-xs font-mono font-semibold bg-stone-100 border border-stone-200 text-stone-800 px-2.5 py-1 rounded-lg"
                  >
                    {e}
                  </span>
                ))}
            ...
            <div className="space-y-2">
              ...
              <div className="flex flex-wrap gap-2">
                {active.targetDegrees.map((d) => (
                  <span
                    key={d}
                    className={`text-[11px] font-mono font-bold px-2.5 py-1 rounded-full border ${trackColor.bg} ${trackColor.border} ${trackColor.text}`}
                  >
                    {d}
                  </span>
                ))}
          ...
          <div className="lg:col-span-6 space-y-6">
                ...
                {active.skillsFrequency.map((item) => (
                  <div key={item.skill} className="space-y-1">
                    <div className="flex justify-between text-xs">
                      ...
                      <span className={`font-mono font-bold ${trackColor.text}`}>
                        {item.percentage}%
                      </span>
                    </div>
            ...
            <div className="space-y-3 bg-stone-50 p-4 rounded-2xl border border-stone-200">
              <span className="text-[10px] font-mono font-bold uppercase text-stone-500 block">
                ARZON MODULES THAT BUILD THIS READINESS
              </span>
              <ul className="space-y-2">
                {active.keyModules.map((mod, i) => (
                  <li
                    key={i}
                    className="flex items-start gap-2 text-xs text-stone-700 font-sans font-medium"
                  >
