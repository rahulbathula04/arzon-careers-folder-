  ...
  return (
    <section id="career-explorer" className="py-16 sm:py-24 border-b border-stone-200 bg-white tone-light">
      <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8 space-y-12">
        ...
        <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-6 gap-2.5">
          {CAREER_TRACKS.map((t) => (
            <button
              ...
              className={`relative p-3.5 rounded-xl text-left border transition-all cursor-pointer flex flex-col justify-between space-y-2 hover:-translate-y-0.5 ${
                activeTrackId === t.id
        ...
        {/* Active Track Deep-Dive Console with Animated Transition */}
        <AnimatePresence mode="wait">
          <motion.div
            ...
            className="rounded-2xl border border-stone-200 bg-[#FAF8F5] p-6 sm:p-10 shadow-xs space-y-8"
          >
            {/* Header Banner */}
                ...
                <div className="rounded-xl bg-white tone-light border border-stone-200 p-4 space-y-2.5 shadow-2xs">
                  ...
                  <div className="flex flex-wrap gap-1.5">
                    {activeTrack.primaryTools.map((tool, idx) => (
                      <span
                        key={idx}
                        className="px-2.5 py-1 rounded-md bg-stone-100 text-stone-800 font-mono text-[11px] font-bold border border-stone-200 hover:border-stone-300 transition-colors"
                      >
                        {tool}
                      </span>
                    ))}
                ...
                <div className="rounded-xl bg-white tone-light border border-stone-200 p-4 space-y-2.5 shadow-2xs">
                  ...
                  <div className="flex flex-wrap gap-1.5">
                    {activeTrack.topEmployers.map((emp, idx) => (
                      <span
                        key={idx}
                        className="px-2.5 py-1 rounded-md bg-sky-50 text-[#1B3F8B] font-mono text-[11px] font-bold border border-sky-200 hover:bg-sky-100 transition-colors"
                      >
                        {emp}
                      </span>
                    ))}
            ...
            <div className="rounded-xl border border-sky-200 bg-sky-50/60 p-4 flex flex-col sm:flex-row sm:items-center justify-between gap-3 text-xs">
              ...
              <div className="flex flex-wrap items-center gap-1.5 font-mono text-[11px]">
                {activeTrack.atsKeywords.map((kw, i) => (
                  <span key={i} className="px-2 py-0.5 rounded bg-white tone-light text-stone-800 border border-stone-200 shadow-2xs">
                    {kw}
                  </span>
                ))}
              </div>
