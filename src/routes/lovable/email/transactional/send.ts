  ...
  server: {
    handlers: {
      POST: async ({ request }) => {
        ...
        // Authorization: caller must have a staff role. Without this, any
        // visitor who can sign up via Supabase auth could send templated
        // emails from the verified sending domain (phishing risk).
        const { data: isStaff, error: roleError } = await supabase.rpc("has_any_role", {
          _user_id: user.id,
