...
const JD_PROVENANCE_BY_SLUG = Object.fromEntries(
  JD_PROVENANCE.map((p) => [p.slug, p])
);
JD_PROVENANCE.reduce((sum, p) => sum + p.jdCount, 0);
const LAST_JD_REFRESH = JD_PROVENANCE.map((p) => p.refreshedOn).sort().reverse()[0];
function refreshQuarter(iso) {
  const d = new Date(iso);
  if (Number.isNaN(d.getTime())) return iso;
  const q = Math.floor(d.getUTCMonth() / 3) + 1;
  return `Q${q} ${d.getUTCFullYear()}`;
}
const RESEARCH_REFRESH_QUARTER = refreshQuarter(LAST_JD_REFRESH);
export {
  JD_PROVENANCE as J,
  RESEARCH_REFRESH_QUARTER as R,
  JD_PROVENANCE_BY_SLUG as a,
  coverageBand as c,
  refreshQuarter as r
};
