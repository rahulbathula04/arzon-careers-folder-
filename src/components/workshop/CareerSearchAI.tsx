  ...
  return (
    <section className="tone-dark bg-[#020617] py-20 text-white border-t border-slate-900/60 relative overflow-hidden">
      ...
      <div className="relative mx-auto max-w-5xl px-4 sm:px-6 lg:px-8">
        ...
        {/* Search Bar Workspace - Transparent Input Lock */}
        <div className="max-w-2xl mx-auto mb-8">
          <div className="relative flex items-center rounded-2xl bg-[#0f172a] p-2 shadow-lg backdrop-blur-md border border-slate-800">
            ...
            <input
              ...
              className="w-full bg-transparent border-none outline-none shadow-none px-3 py-2.5 text-sm text-white placeholder-slate-400 focus:outline-none focus:ring-0 focus:border-none font-sans"
            />
            <button
              type="button"
          ...
          <div className="mt-3 flex flex-wrap justify-center gap-2">
            {sampleQueries.map((sq) => (
              <button
                ...
                type="button"
                onClick={() => setQuery(sq)}
                className="text-[11px] font-sans font-semibold text-slate-200 bg-[#080d1a] hover:bg-slate-800 px-3.5 py-1.5 rounded-full transition-colors border border-slate-800 cursor-pointer"
              >
                {sq}
              </button>
            ))}
                  ...
                  {isOpen && (
                    ...
                    >
                      <div className="px-6 pb-5 pt-1 text-xs text-slate-300 leading-relaxed font-sans border-t border-slate-800/60">
                        <p>{faq.a}</p>
                        <div className="mt-3 inline-flex items-center gap-1.5 text-[11px] font-mono text-emerald-400 bg-emerald-500/10 px-2.5 py-1 rounded-md border border-emerald-500/20 font-semibold">
