...
var Tokenizer = (_b = class {
  static get() {
    return _b._pool.pop() || new _b();
  }
  static release(tokenizer) {
    if (_b._pool.length < _b.MAX_POOL_SIZE) {
      tokenizer._reset();
      _b._pool.push(tokenizer);
    }
  }
  constructor() {
    this._value = [];
...
}, _b._pool = [], _b.MAX_POOL_SIZE = 40, _b);
...
const _a2 = 1.09929682680944;
const _b2 = 0.018053968510807;
const rec20202rec2020Linear = (rgb2) => {
  return rgb2.map(function(c) {
    return c < _b2 * 4.5 ? c / 4.5 : Math.pow((c + _a2 - 1) / _a2, 1 / 0.45);
  });
};
const rec2020Linear2rec2020 = (rgb2) => {
  return rgb2.map(function(c) {
    return c >= _b2 ? _a2 * Math.pow(c, 0.45) - (_a2 - 1) : 4.5 * c;
  });
};
...
const _color = (context, args) => {
  ...
  if (!isRelativeTransform(tokens)) {
    ...
    if (isIdentToken(tokens[1])) if (typeof COLORS[tokens[1].value.toUpperCase()] === "undefined") throw new Error(`Attempting to use unknown color in relative color 'from'`);
      ...
      const _c2 = parseColor(context, tokens[1].value), alpha = 255 & _c2, blue = 255 & _c2 >> 8, green = 255 & _c2 >> 16;
      from = [
        {
          type: 17,
          number: 255 & _c2 >> 24,
...
var CSSParsedDeclaration = (_c = class {
  get border() {
  ...
  constructor(context, declaration) {
    const standardProps = _c.standardProps;
...
var BackgroundRenderer = (_d = class {
  constructor(deps) {
  ...
  */
  async renderBackgroundURLImage(container, backgroundImage2, index) {
    ...
    try {
      ...
      this.context.onError?.(e2 instanceof Error ? e2 : new Error(String(e2)));
    }
    if (image2) {
  ...
  */
  ...
  lruSet(cache, key, value) {
    if (cache.size >= _d.PATTERN_CACHE_MAX) {
      const oldestKey = cache.keys().next().value;
      cache.delete(oldestKey);
    }
    cache.set(key, value);
  }
}, _d.PATTERN_CACHE_MAX = 50, _d);
...
const calculateObjectFitRendering = (intrinsicWidth, intrinsicHeight, box, objectFit2, objectPosition2) => {
  let sx = 0;
  let sy = 0;
...
async function renderReplacedElements(ctx, context, options, iframeRendererFactory, container, curves, styles, renderReplacedElementFn) {
  if (container instanceof ImageElementContainer) try {
    ...
    context.onError?.(e2 instanceof Error ? e2 : new Error(String(e2)));
  }
...
var Context = (_f = class {
  constructor(options, windowBounds, config) {
    this.windowBounds = windowBounds;
    this.instanceName = `#${_f.instanceCount++}`;
    this.config = config;
