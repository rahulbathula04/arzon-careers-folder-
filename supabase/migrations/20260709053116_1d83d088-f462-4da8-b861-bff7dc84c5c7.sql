...
BEGIN
  ...
  -- Rate limit: max 5 track requests per hour per phone (ce_rate_hit returns TRUE when over limit)
  ...
  RETURN jsonb_build_object('ok', true, 'created', v_created, 'slug', v_slug, 'duplicateVote', v_duplicate_vote);
END;
$$;

CREATE OR REPLACE FUNCTION public.record_recommendation_outcome(
  p_lead_id uuid, p_family_id text, p_recommended_role_slug text,
  p_chosen_role_slug text, p_stage text
) RETURNS jsonb LANGUAGE plpgsql SECURITY DEFINER SET search_path = public AS $$
DECLARE
  v_existing_user uuid; v_lead_exists boolean;
BEGIN
  IF p_lead_id IS NULL THEN RAISE EXCEPTION 'invalid_lead_id'; END IF;
  IF p_family_id IS NULL OR char_length(p_family_id) > 120 THEN RAISE EXCEPTION 'invalid_family_id'; END IF;
  IF p_recommended_role_slug IS NOT NULL AND char_length(p_recommended_role_slug) > 120 THEN RAISE EXCEPTION 'invalid_recommended_slug'; END IF;
  ...
  SELECT EXISTS(SELECT 1 FROM career_engine_leads WHERE id = p_lead_id AND deleted_at IS NULL) INTO v_lead_exists;
  IF NOT v_lead_exists THEN RAISE EXCEPTION 'lead_not_found'; END IF;
  ...
  ON CONFLICT (lead_id) DO UPDATE SET
    ...
    updated_at            = now()
  WHERE recommendation_outcomes.user_id IS NULL OR recommendation_outcomes.user_id = auth.uid();
