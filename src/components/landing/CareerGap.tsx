...
const GAP_STAGES = [
  {
    label: "YOUR DEGREE",
    detail: "B.Pharm / Pharm.D / B.Sc / Biotech",
    color: "bg-stone-100 text-stone-700 border-stone-300",
  },
  {
    label: "ACADEMIC KNOWLEDGE",
    detail: "Theory, concepts, exams",
    color: "bg-stone-100 text-stone-700 border-stone-300",
  },
  {
    label: "THE SKILL GAP",
    detail: "Where most freshers get stuck",
    color: "bg-rose-50 text-rose-700 border-rose-300",
    highlight: true,
  },
  {
    label: "PRACTICAL CAPABILITY",
    detail: "Real tools, real files, real workflows",
    color: "bg-sky-50 text-[#1B3F8B] border-sky-300",
  },
  {
    label: "EMPLOYER READINESS",
    detail: "Hired into PV, Coding, CDM, CRA",
      ...
      <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8 space-y-12">
        ...
        <div className="text-center space-y-4 max-w-3xl mx-auto">
          ...
          <p className="text-sm sm:text-base text-stone-700 leading-relaxed font-sans font-medium">
            Indian healthcare employers hiring freshers for Pharmacovigilance, Medical Coding, and
            CDM roles don't just want a degree. They test specific, practical skills on day one —
            skills that no university syllabus covers.
          </p>
        </div>
        ...
        <div className="flex flex-col items-center gap-1 w-full max-w-xl mx-auto px-2">
          {GAP_STAGES.map((stage, idx) => (
            <div key={idx} className="flex flex-col items-center w-full">
              ...
              >
                <div
                  className={`font-mono font-black text-xs sm:text-sm tracking-wider uppercase ${stage.highlight ? "text-rose-700" : ""}`}
                >
                  {stage.label}
                </div>
                <div className="text-[10px] sm:text-[11px] font-sans font-medium mt-0.5 opacity-80">
                  {stage.detail}
                </div>
              </div>
              {idx < GAP_STAGES.length - 1 && (
                <div
                  className={`h-4 sm:h-5 w-0.5 ${stage.highlight ? "bg-rose-300" : "bg-stone-300"}`}
                />
              )}
            </div>
          ))}
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 max-w-5xl mx-auto">
          ...
          <div className="bg-white tone-light card-light rounded-2xl border border-stone-200 p-5 sm:p-8 space-y-5 shadow-xs">
            <div className="border-b border-stone-100 pb-4">
              ...
              <h3 className="font-serif font-bold text-lg sm:text-xl text-[#1A1A1A] mt-1">
                Academic Foundation
              </h3>
            </div>
            <ul className="space-y-3">
              {COLLEGE_GIVES.map((item, i) => (
                <li
                  key={i}
                  className="flex items-start gap-2.5 sm:gap-3 text-xs sm:text-sm text-stone-600 font-sans font-medium"
                >
          ...
          <div className="bg-white tone-light card-light rounded-2xl border-2 border-[#1B3F8B] p-5 sm:p-8 space-y-5 shadow-md">
            <div className="border-b border-stone-100 pb-4 flex items-center justify-between">
              <div>
                <span className="font-mono text-xs font-bold uppercase tracking-wider text-[#1B3F8B] block">
            ...
            <ul className="space-y-3">
              {EMPLOYERS_EXPECT.map((item, i) => (
                <li
                  key={i}
                  className="flex items-start gap-2.5 sm:gap-3 text-xs sm:text-sm text-[#1A1A1A] font-sans font-semibold"
                >
            ...
            <div className="bg-[#1B3F8B] rounded-xl p-3.5 sm:p-4">
              <p className="text-xs font-sans font-bold text-center" style={{ color: "#FFFFFF" }}>
                Arzon builds these exact skills in 12 weeks through live training + applied
                internship
              </p>
            </div>
          </div>
